    ...
    let raw: RawMessage = match serde_json::from_str(json_str) {
        ...
        Err(err) => {
            ...
            return vec![NormalizedRow {
                ulid: Some(generate_dedup_ulid(Utc::now().timestamp_millis() as u64, json_str.as_bytes()).to_string()),
...
pub fn create_arrow_record_batch(rows: &[NormalizedRow]) -> Result<RecordBatch> {
    let ulid_arr = StringArray::from(rows.iter().map(|r| r.ulid.clone().unwrap_or_default()).collect::<Vec<String>>(),);
